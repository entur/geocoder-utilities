...
public class ParentsConverter<T, I> extends AbstractBeanField<T, I> {
  ...
  protected Parents convert(String value) throws CsvDataTypeMismatchException {
    ...
    if (matcher.matches()) {
      ...
      Pattern parentsPattern = Pattern.compile(
        "(,\\W)?(?<parentType>.*?)(?<sep>[=])\\[(?<parentFields>.*?)].*?"
      );
      ...
      while (parentsMatcher.find()) {
        ...
        Pattern ParentFieldsPattern = Pattern.compile(
          "peliasId\\((?<peliasId>.*?)\\)(?<sep>[|])name\\((?<name>.*?)\\)(?:|abbr\\((?<abbr>.*?)\\))?"
        );
        ...
        if (parentFieldsMatcher.matches()) {
          ...
          Pattern peliasIdPattern = Pattern.compile(
      ...
      throw new CsvDataTypeMismatchException(
        "Conversion of [" + value + "] to Parents failed."
      );
    }
  }
}
